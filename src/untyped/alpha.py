...
def substitution(
    ...
    old: Variable,
    new: LambdaTerm,
    bounded_variables: Optional[List[Variable]] = None,
) -> LambdaTerm:
    ...
    match term:
        case Variable(_):
            assert term not in bounded_variables, (
                f"Error in implementation. {term} should be renamed before it is "
                "reached. "
            )
            return new if term == old else term

        case Application(of, on):
            return Application(substitution(of, old, new), substitution(on, old, new))
        case Abstraction(bound_var, expr):
            assert bound_var not in bounded_variables, (
                f"Badly constructed lambda expression. {bound_var} is already "
                "bounded. "
            )
            if bound_var == old:
            ...
            return Abstraction(bound_var, substitution(expr, old, new))
